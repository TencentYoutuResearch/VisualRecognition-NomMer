...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., depth=-1):
        super().__init__()
        self.dim = dim
        self.depth = depth
...
def init_dct_kernel(in_ch, ksize=8, rsize=2):
    ...
    for v in range(rsize):
        for u in range(rsize):
            kernel = (2*C[v]*C[u]/ksize)*np.cos((2*YY+1)*v*np.pi/(2*ksize))*np.cos((2*XX+1)*u*np.pi/(2*ksize))
        ...
        for u in range(rsize):
            for j in range(ksize):
                for i in range(ksize):
                    kernel = (2*C[v]*C[u]/ksize)*np.cos((2*j+1)*v*np.pi/(2*ksize))*np.cos((2*i+1)*u*np.pi/(2*ksize))
...
class DCTAttention(nn.Module):
    def __init__(self, dim, ksize, heads):
        ...
        self.idct_conv = nn.Sequential(
                                init_idct_kernel(dim, ksize, reserve_kernel),
                                nn.PixelShuffle(ksize)
                                )
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout = 0., drop_path=0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        assert(dim%heads == 0)
        for n in range(depth):
...
class NomMerAttn(nn.Module):
    ...
    def __init__(self, emd_dim=128, depths=[2,2,16,2], num_heads=[2,4,8,16], input_size=224, win_size=7, \
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(emd_dim*8),
            nn.Linear(emd_dim*8, self.num_class)
        )
